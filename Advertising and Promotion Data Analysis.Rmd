---
title: <center> <h3> Spring 2022 </h3> <h2> GE 461 Data Science </h2> <h3> taught by Savaş Dayanık </h3>  <h2> Data Analysis Project  </h2>  <h3> done by </h3> <h4> <span style="color:blue">* Caner Canlıer * </span> </h4> </center>
...
It seems like cap is not statistically significant since p value is large. We can drop it manually.

```{r}
simple<- update(simple,.~.-cap)
```
...
To measure the size of the effect, lets construct confidence interval:
```{r}
simple %>% confint(parm = "bobbleheadYES", level = 0.90)
simple %>% confint(parm = "fireworksYES", level = 0.90)
```

Seems like if we give bobblehead attendance will be increase between 6400-14119 with 0.9 CI.
also if there is a firework show, attendance tend to increase between 7026-27660 with 0.9 CI.
We can say that if giving booblehead and doing firework show cost us the same, we would choose firework.(of course it is for the night games.)
...
For instance, if we want to predict the attendance in June friday while temp is avearge and no cap or bobblehead is given.
    ...
    ```{r}
newdata = data.frame(month = "JUN",day_of_week="Friday",bobblehead="NO",fireworks="NO",temp=mean(events$temp))
predict(simple, newdata = newdata, interval = "prediction", level = 0.90)

```

According to our prediction, with 90% confidence level, we can say that we will get 30048 attendance.

**TO SUM UP**, According to our findings, we conclude that month, day of week, firework show, temperature and giving bobbleheads have impact on attendance. People tend to attend games more often in warm days (approximately 24 Celsius) and this temperature corresponds to June. Also game played in tuesday,saturday and sunday get more attendance so company can consider schedule games in these days. Besides date, firework shows can increase attendance. Also by giving bobblehead or displaying a firework show can increase our attendance approximately 10300 and 17300, respectively.